...
class MusicPlayer(commands.Cog, name='Music'):
    ...
    async def queue(self, msg, song):
        ...
        """
        tuplE = await Downloader.get_info(self, url=song)
        title = tuplE[0]['title']
        data = tuplE[1]
        # NOTE:needs fix here
        if data['queue']:
    ...
    async def start_song(self, msg, song):
        ...
        emb.set_footer(
            text=f'Добавил {msg.author.display_name}', icon_url=msg.author.avatar_url)
        loop = asyncio.get_event_loop()
    ...
    async def before_play(self, msg):
        ...
        if msg.author.voice is None:
            return await msg.send('**Подключитесь к каналу, чтобы вопроизвести музыку**'.title())
        ...
        if msg.voice_client.channel != msg.author.voice.channel:
            ...
            if self.player[msg.guild.id]['queue']:
                ...
                return await msg.send("Вы должны быть в том же канале, что и бот.")
    ...
    async def repeat(self, msg):
        ...
        if msg.guild.id in self.player:
            ...
            return await msg.send("Сейчас не играет музыка")
        return await msg.send("Бот не в голсовом канале")
    ...
    async def reset(self, msg):
        ...
        """
        if msg.voice_client is None:
            return await msg.send(f"**{msg.author.display_name}, сейчас не играет музыка.**")

        if msg.author.voice is None or msg.author.voice.channel != msg.voice_client.channel:
            return await msg.send(f"**{msg.author.display_name}, вы должны быть в том же канале, что и бот.**")

        if self.player[msg.guild.id]['queue'] and msg.voice_client.is_playing() is False:
            return await msg.send("**Нет музыки**".title(), delete_after=25)

        self.player[msg.guild.id]['reset'] = True
        msg.voice_client.stop()

    #@commands.has_permissions(manage_channels=True)
    @command(aliases=['ылшз'])
    async def skip(self, msg):
        ...
        """
        if msg.voice_client is None:
            return await msg.send("**Сейчас не играет музыка**".title(), delete_after=60)

        if msg.author.voice is None or msg.author.voice.channel != msg.voice_client.channel:
            return await msg.send("Подключитесь к тому же голосовому каналу, что и бот")

        if not self.player[msg.guild.id]['queue'] and msg.voice_client.is_playing() is False:
            return await msg.send("**Нет музыки для скипа**".title(), delete_after=60)
    ...
    async def stop(self, msg):
        ...
        """
        if msg.voice_client is None:
            return await msg.send("Бот не подключен к голосовому каналу")
    ...
    async def pause(self, msg):
        ...
        if msg.author.voice is not None and msg.voice_client is not None:
            if msg.voice_client.is_paused() is True:
                return await msg.send("Трек уже стоит на паузе")
    ...
    async def resume(self, msg):
        ...
        if msg.author.voice is not None and msg.voice_client is not None:
            if msg.voice_client.is_paused() is False:
                return await msg.send("Трек уже играет")
    ...
    async def song_info(self, msg):
        ...
        return await msg.send(f"**Нет треков длч воспроизведения**".title(), delete_after=30)
    ...
    async def join_error(self, msg, error):
        ...
        if error.args[0] == 'Command raised an exception: Exception: playing':
            return await msg.send("**Подключитесь к голосовому каналу**".title())
    ...
    async def download(self, ctx, *, song):
        ...
        try:
            ...
            embed = discord.Embed(title="Song couldn't be downloaded \nMay be song is too large for discord.", description=("Song:"+song))
